...
async def get_email_service_async(db):
    ...
    return EmailService(
        ...
        app_name=s.get("app_name", s.get("org_name", "CropDoc")),
    )
...
async def register(data: dict, background_tasks: BackgroundTasks, db=Depends(get_db)):
    ...
    user = {
        ...
        "role": data.get("role", "user"), "is_active": True, "email_verified": False,
        "email_otp": otp, "email_otp_expiry": (datetime.now(timezone.utc) + timedelta(minutes=10)).isoformat(),
        "reset_otp": None, "reset_otp_expiry": None, "refresh_token": None,
...
async def resend_otp(data: dict, background_tasks: BackgroundTasks, db=Depends(get_db)):
    ...
    otp = generate_otp()
    await db.users.update_one({"_id": user["_id"]}, {"$set": {"email_otp": otp, "email_otp_expiry": (datetime.now(timezone.utc) + timedelta(minutes=10)).isoformat()}})
    email_svc = await get_email_service_async(db)
    if email_svc:
...
async def login(data: dict, db=Depends(get_db)):
    ...
    await db.users.update_one({"_id": user["_id"]}, {"$set": {"refresh_token": refresh_token, "last_login_at": datetime.now(timezone.utc)}})
    return {
        "success": True, "token": access_token, "refresh_token": refresh_token,
        "user": {"id": str(user["_id"]), "email": user["email"], "name": user.get("name", ""), "role": user.get("role", "user"), "email_verified": user.get("email_verified", False)},
    }
...
async def forgot_password(data: dict, background_tasks: BackgroundTasks, db=Depends(get_db)):
    ...
    otp = generate_otp()
    await db.users.update_one({"_id": user["_id"]}, {"$set": {"reset_otp": otp, "reset_otp_expiry": (datetime.now(timezone.utc) + timedelta(minutes=10)).isoformat()}})
...
async def reset_password(data: dict, background_tasks: BackgroundTasks, db=Depends(get_db)):
    ...
    await db.users.update_one({"_id": user["_id"]}, {"$set": {"password_hash": hash_password(new_password), "reset_otp": None, "reset_otp_expiry": None, "refresh_token": None}})
...
async def change_password(data: dict, background_tasks: BackgroundTasks, user=Depends(get_current_user), db=Depends(get_db)):
    ...
    await db.users.update_one({"_id": ObjectId(user["id"])}, {"$set": {"password_hash": hash_password(new_password), "refresh_token": None}})
    if await should_send_email(db, "password_changed"):
        email_svc = await get_email_service_async(db)
        if email_svc:
...
async def me(user=Depends(get_current_user)):
    return {"success": True, "user": {"id": user["id"], "email": user["email"], "name": user.get("name", ""), "role": user.get("role", "user"), "email_verified": user.get("email_verified", False)}}
