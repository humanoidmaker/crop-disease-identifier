...
async def update_email_config(data: dict, user=Depends(require_admin), db=Depends(get_db)):
    allowed_keys = ["smtp_host", "smtp_port", "smtp_user", "smtp_pass", "smtp_from", "email_verification_enabled", "email_welcome_enabled", "email_password_reset_enabled", "email_password_changed_enabled", "require_email_verification"]
    for k, v in data.items():
...
async def test_email(data: dict, user=Depends(require_admin), db=Depends(get_db)):
    ...
    svc = EmailService(host=host, port=int(s.get("smtp_port", "587")), user=s.get("smtp_user", ""), password=s.get("smtp_pass", ""), from_addr=s.get("smtp_from", s.get("smtp_user", "")), app_name=s.get("app_name", "CropDoc"))
    to = data.get("to", user["email"])
    result = svc.send_custom(to, "Test Email", f"This is a test email from {s.get('app_name', 'CropDoc')}. SMTP is working correctly.")
